...
@section('content')
    <div class="p-4 sm:p-6 space-y-6 md:space-y-10" x-data="{ showSiswaModal: false, modalTitle: '', modalStudents: [] }">

        <div
            class="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-br from-emerald-600 to-teal-700 p-6 sm:p-10 shadow-2xl">
            <div class="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
                <div class="text-center lg:text-left max-w-2xl">
                    ...
                    <p class="text-emerald-50 text-base sm:text-lg opacity-90 mb-8">
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap justify-center lg:justify-start gap-4">
                        <a href="#jadwal"
                            class="bg-white text-emerald-700 px-8 py-3 rounded-2xl font-bold hover:bg-emerald-50 transition-all shadow-lg flex items-center justify-center gap-2 w-full sm:w-auto">
                            <i class="fas fa-calendar-day"></i> Lihat Jadwal Hari Ini
                        </a>
                        @auth
                            <a href="{{ route('dashboard') }}"
                                class="bg-emerald-800/40 text-white border border-emerald-400/30 px-8 py-3 rounded-2xl font-bold hover:bg-emerald-800/60 transition-all text-center w-full sm:w-auto">
                                Dashboard Admin
                            </a>
                        @else
                            <a href="{{ route('login') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
            <div
                class="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm text-center group hover:border-emerald-500 transition-all">
                <div
                ...
                <h3 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white">{{ $stats['total_siswa'] }}</h3>
                <p class="text-xs sm:text-sm font-bold text-gray-500 uppercase tracking-widest mt-1">Siswa Terdaftar</p>
            </div>
            ...
            <div
                ...
                <h3 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white">{{ $stats['kelas_aktif'] }}</h3>
                <p class="text-xs sm:text-sm font-bold text-gray-500 uppercase tracking-widest mt-1">Sesi Aktif Hari Ini</p>
            </div>

            <div
                class="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm text-center group hover:border-emerald-500 transition-all sm:col-span-2 lg:col-span-1">
                <div
                ...
                <h3 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white">{{ $stats['pengajar'] }}</h3>
                <p class="text-xs sm:text-sm font-bold text-gray-500 uppercase tracking-widest mt-1">Tenaga Pendidik</p>
            </div>
        </div>

        <div id="jadwal" class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            <div
                class="lg:col-span-2 bg-white dark:bg-gray-800 p-4 sm:p-8 rounded-2xl sm:rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
                    <h4 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                        <span class="w-2 h-8 bg-emerald-500 rounded-full"></span>
                        Aktivitas Belajar Hari Ini
                    </h4>
                    <span
                        class="px-4 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-xl text-xs font-bold text-gray-500 self-start sm:self-auto">
                ...
                <div class="space-y-4 sm:space-y-6">
                    @forelse($listJadwal as $namaSesi => $jadwals)
                        <div
                            class="flex flex-col md:flex-row gap-4 sm:gap-6 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 shadow-sm">
                            <div
                                class="flex-shrink-0 w-full md:w-24 text-center border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700 pb-4 md:pb-0 md:pr-6 flex md:flex-col justify-between md:justify-center items-center gap-1">
                                <span
                                    class="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Waktu</span>
                                <div class="text-lg sm:text-xl font-black text-gray-900 dark:text-white mt-0 md:mt-1">
                                    {{ $namaSesi }}</div>
                                <div class="text-[10px] text-gray-400 font-medium md:mt-0">
                                    ...
                                    <div @click="modalTitle = '{{ $j->mataPelajaran->name }}'; modalStudents = {{ $slotStudents->toJson() }}; showSiswaModal = true"
                                        ...
                                        <div class="flex items-start justify-between gap-2">
                                            <div class="min-w-0 flex-1">
                                                <div
                                                    class="font-bold text-sm text-gray-800 dark:text-white leading-tight break-words">
                                                    {{ $j->mataPelajaran->name }}
                                                </div>
                                                <div
                                                    class="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold mt-1 flex items-center gap-1 truncate">
                                                    <i class="fas fa-user-tie text-[9px] flex-shrink-0"></i>
                                                    <span class="truncate">{{ $j->guru->name }}</span>
                                                </div>
                                            </div>
                                            <div
                                                class="text-[10px] bg-gray-100 dark:bg-gray-600 px-2 py-1 rounded-lg text-gray-500 dark:text-gray-300 font-bold whitespace-nowrap flex-shrink-0">
                                                {{ $j->ruang->name }}
                                            </div>
                                        </div>
                                    </div>
                    ...
                    @empty
                        <div
                            class="text-center py-16 sm:py-20 bg-gray-50 dark:bg-gray-800/50 rounded-2xl sm:rounded-3xl border border-dashed border-gray-200 dark:border-gray-700">
                            <i class="fas fa-mug-hot text-gray-300 fa-3x mb-4"></i>
                            <p class="text-gray-400 italic text-sm sm:text-base">Tidak ada jadwal belajar untuk hari ini.
                            </p>
                        </div>
                    @endforelse
                </div>
            ...
            <div class="space-y-6">
                <div
                    class="bg-emerald-50 dark:bg-emerald-900/10 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-emerald-100 dark:border-emerald-800/30">
                    <div
