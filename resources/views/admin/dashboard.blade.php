<x-admin-layout :active-tab="$activeTab">
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-600 dark:text-emerald-400">
                    Control Center</p>
                <h2 class="text-xl font-black tracking-tight text-slate-900 dark:text-white sm:text-2xl">Dashboard
                    Operasional</h2>
            </div>
            <div
                class="mt-2 inline-flex items-center gap-2 self-start rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/50 dark:text-emerald-300 sm:mt-0">
                <span class="relative flex h-2 w-2"><span
                        ...
                        class="relative inline-flex h-2 w-2 rounded-full bg-emerald-500"></span></span>
                Sistem aktif
            </div>
        </div>
    </x-slot>
                        ...
                        <tbody class="bg-white dark:bg-gray-800">
                            @foreach ($sesis->sortBy('start_time') as $sesi)
                                <tr x-show="sessionMatches({{ $sesi->id }})"
                                    class="even:bg-gray-50/50 dark:even:bg-gray-800/60">
                                    <td
                                            ...
                                            @if (isset($jadwals[$hari->id][$sesi->id]))
                                                @foreach ($jadwals[$hari->id][$sesi->id] as $groupedClass)
                                                    @php
                                                        ...
                                                        $cardBgColor = $siswaCount < 4 ? 'bg-white/100' : 'bg-white/90';
                                                    @endphp
