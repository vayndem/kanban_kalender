        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="container mx-auto" x-data="{
                ...
                deleteDataItem(id) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            let endpoint = '';
                            if (this.currentForm === 'mapel') endpoint = '{{ route('admin.mapel.destroy', ':id') }}';
                            else if (this.currentForm === 'guru') endpoint = '{{ route('admin.guru.destroy', ':id') }}';
                            else if (this.currentForm === 'ruang') endpoint = '{{ route('admin.ruang.destroy', ':id') }}';
                            else if (this.currentForm === 'sesi') endpoint = '{{ route('admin.sesi.destroy', ':id') }}';
                            else if (this.currentForm === 'siswa') endpoint = '{{ route('admin.siswa.destroy', ':id') }}';
                            else if (this.currentForm === 'tanda') endpoint = '{{ route('admin.tanda.destroy', ':id') }}';
                ...
                saveNewData() {
                    ...
                    if (formType === 'mapel') endpoint = isEdit ? '{{ route('admin.mapel.update', ':id') }}' : '{{ route('admin.mapel.store') }}';
                    else if (formType === 'guru') endpoint = isEdit ? '{{ route('admin.guru.update', ':id') }}' : '{{ route('admin.guru.store') }}';
                    else if (formType === 'ruang') endpoint = isEdit ? '{{ route('admin.ruang.update', ':id') }}' : '{{ route('admin.ruang.store') }}';
                    else if (formType === 'sesi') endpoint = isEdit ? '{{ route('admin.sesi.update', ':id') }}' : '{{ route('admin.sesi.store') }}';
                    else if (formType === 'siswa') endpoint = isEdit ? '{{ route('admin.siswa.update', ':id') }}' : '{{ route('admin.siswa.store') }}';
                    else if (formType === 'tanda') endpoint = isEdit ? '{{ route('admin.tanda.update', ':id') }}' : '{{ route('admin.tanda.store') }}';
                    ...
                    <div class="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-lg mb-6">
                        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-6">
                            ...
                            <div class="flex-grow max-w-2xl">
                                <label for="universalSearch" class="block text-sm font-medium text-gray-700 dark:text-white mb-1">
                                    <i class="fas fa-search mr-1"></i> Pencarian Universal
                                </label>
                                <div class="relative rounded-md shadow-sm">
                                    <input type="text"
                                        id="universalSearch"
                                        x-model.debounce.300ms="universalSearch"
                                    ...
                                    <button x-show="universalSearch.length > 0"
                                            @click="universalSearch = ''"
                                                ...
                                                @if (isset($jadwals[$hari->id][$sesi->id]))
                                                    @foreach ($jadwals[$hari->id][$sesi->id] as $groupedClass)
                                                        ...
                                                        <div class="kanban-card group relative bg-white/90 dark:bg-gray-700/90 backdrop-blur-sm p-2.5 mb-2 rounded-lg shadow border-l-4 text-sm cursor-move transition-all duration-200 ease-out hover:shadow-xl hover:-translate-y-1"
                                                            ...
                                                            :class="{
                                                                'opacity-30': universalSearch !== '' && !'{{ $searchableText }}'.includes(universalSearch.toLowerCase())
                                                            }"
                                                            @click.stop>
                                                    ...
                                                    <template x-if="hasTanda(selectedStudentDetail)">
                                                        <ul class="space-y-3">
                                                            <template x-for="tanda in selectedStudentDetail.tandas" :key="tanda.id">
                                                                <li class="relative bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 p-3 rounded shadow-sm text-sm text-gray-800 dark:text-white mb-2">
                                                                    <div class="pr-8">
                                                                        <p x-text="tanda.keterangan" class="break-words"></p>
                                                                        <span class="text-xs text-gray-400 mt-1 block" x-text="new Date(tanda.created_at).toLocaleDateString()"></span>
                                                                    </div>
                                                                    <button type="button"
