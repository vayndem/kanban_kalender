...
<div class="space-y-6">
    ...
    <div x-data="{ cards: @js($kelasHariIni), selectedCard: null }">
        <h3 class="text-sm font-black uppercase tracking-wide text-slate-500 dark:text-slate-400 mb-3">Jadwal Hari Ini</h3>

        <template x-if="cards.length === 0">
            <div class="rounded-xl border border-dashed border-gray-200 bg-white p-8 text-center text-sm italic text-slate-400 dark:border-gray-700 dark:bg-gray-800">
            ...
            <template x-for="(card, idx) in cards" :key="idx">
                <button type="button" @click="selectedCard = card"
                    ...
                    <div class="flex items-center justify-between">
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-black text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300">
                            <i class="fas fa-clock"></i> <span x-text="card.sesi_name"></span>
                        </span>
                        <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-black text-slate-600 dark:bg-slate-700 dark:text-slate-300">
                            <i class="fas fa-user-group"></i> <span x-text="card.siswa_list.length"></span>
                        </span>
                    </div>

                    <div class="mt-3 text-base font-black text-slate-900 group-hover:text-emerald-600 dark:text-white dark:group-hover:text-emerald-400" x-text="card.mapel_name"></div>

                    <div class="mt-2 space-y-1 text-xs font-semibold text-slate-500 dark:text-slate-400">
                        <div class="flex items-center gap-1.5"><i class="fas fa-chalkboard-user w-3.5 text-slate-400"></i> <span x-text="card.guru_name"></span></div>
                        <div class="flex items-center gap-1.5"><i class="fas fa-door-open w-3.5 text-slate-400"></i> <span x-text="card.ruang_name"></span></div>
                    </div>
                    ...
                    <div class="mt-3 flex flex-wrap gap-1 border-t border-gray-100 pt-3 dark:border-gray-700">
                        <template x-for="siswa in card.siswa_list.slice(0, 4)" :key="siswa.name">
                            <span class="rounded-md bg-slate-50 px-1.5 py-0.5 text-[10px] font-bold text-slate-600 dark:bg-slate-700/60 dark:text-slate-300" x-text="siswa.panggilan || siswa.name"></span>
                        </template>
                        <span x-show="card.siswa_list.length > 4" class="rounded-md bg-slate-50 px-1.5 py-0.5 text-[10px] font-bold text-slate-400 dark:bg-slate-700/60"
        ...
        <template x-if="selectedCard">
            <div x-show="selectedCard" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" @click="selectedCard = null">
                <div @click.stop x-transition class="w-full max-w-lg overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800">
                    <div class="flex items-start justify-between gap-4 bg-gradient-to-r from-emerald-600 to-teal-600 p-5 text-white">
                        <div>
                            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-100" x-text="selectedCard.sesi_name"></p>
                        ...
                        <button type="button" @click="selectedCard = null" class="rounded-full bg-white/15 px-3 py-2 text-sm font-bold hover:bg-white/25">Tutup</button>
                    </div>
                    <div class="max-h-[50vh] overflow-y-auto p-5">
                        <p class="mb-3 text-xs font-black uppercase tracking-wider text-slate-400">Daftar Siswa (<span x-text="selectedCard.siswa_list.length"></span>)</p>
                        <div class="space-y-2">
                            <template x-for="siswa in selectedCard.siswa_list" :key="siswa.name">
                                <div class="flex items-center justify-between rounded-xl border border-gray-100 bg-gray-50 px-3 py-2 dark:border-gray-700 dark:bg-gray-900/40">
                                    <span class="text-sm font-bold text-slate-800 dark:text-slate-100" x-text="siswa.name"></span>
                                    <span class="text-xs font-semibold text-slate-400" x-text="siswa.kelas || '-'"></span>
                                </div>
                            </template>
                        </div>
                    </div>
    ...
    <div class="flex items-center justify-between">
        <h3 class="text-sm font-black uppercase tracking-wide text-slate-500 dark:text-slate-400">Statistik Operasional</h3>
    ...
    <div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div class="overflow-hidden rounded-2xl border border-t-4 border-t-rose-400 border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:border-t-rose-500 dark:bg-gray-800">
                <div class="flex items-center gap-3 border-b border-gray-100 bg-rose-50/60 px-4 py-3 dark:border-gray-700 dark:bg-rose-950/20">
                    <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-rose-500 text-white shadow-sm"><i class="fas fa-file-invoice"></i></span>
                    <div>
                        <p class="text-xs font-black text-rose-700 dark:text-rose-300">Belum Ditagih Bulan Ini</p>
                        <p class="text-lg font-black text-slate-900 dark:text-white">{{ $finansial['belum_ditagih']->count() }}</p>
                    </div>
                </div>
                <div class="max-h-52 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($finansial['belum_ditagih'] as $item)
                        <div class="flex items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs dark:bg-gray-900/40">
                            <i class="fas fa-user text-[10px] text-rose-400"></i>
                            <span class="font-bold text-slate-700 dark:text-slate-200">{{ $item['siswa_name'] }}</span>
            ...
            <div class="overflow-hidden rounded-2xl border border-t-4 border-t-amber-400 border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:border-t-amber-500 dark:bg-gray-800">
                <div class="flex items-center gap-3 border-b border-gray-100 bg-amber-50/60 px-4 py-3 dark:border-gray-700 dark:bg-amber-950/20">
                    <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-500 text-white shadow-sm"><i class="fas fa-hourglass-half"></i></span>
                    <div>
                        <p class="text-xs font-black text-amber-700 dark:text-amber-300">Piutang Lama &gt; {{ $finansial['piutang_bulan'] }} Bulan</p>
                        <p class="text-lg font-black text-slate-900 dark:text-white">{{ $finansial['piutang_lama']->count() }}</p>
                    </div>
                </div>
                <div class="max-h-52 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($finansial['piutang_lama'] as $item)
                        <div class="flex items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs dark:bg-gray-900/40">
                            ...
                            <span class="font-bold text-slate-700 dark:text-slate-200">{{ $item['siswa_names'] ?: $item['no_hp'] }}</span>
            ...
            <div class="overflow-hidden rounded-2xl border border-t-4 border-t-indigo-400 border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:border-t-indigo-500 dark:bg-gray-800">
                <div class="flex items-center gap-3 border-b border-gray-100 bg-indigo-50/60 px-4 py-3 dark:border-gray-700 dark:bg-indigo-950/20">
                    <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-500 text-white shadow-sm"><i class="fas fa-tags"></i></span>
                    <div>
                        <p class="text-xs font-black text-indigo-700 dark:text-indigo-300">Diskon Menggantung</p>
                        <p class="text-lg font-black text-slate-900 dark:text-white">{{ $finansial['diskon_menggantung']->count() }}</p>
                    </div>
                </div>
                <div class="max-h-52 space-y-1.5 overflow-y-auto p-3">
    ...
    <div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div class="overflow-hidden rounded-2xl border border-t-4 border-t-sky-400 border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:border-t-sky-500 dark:bg-gray-800">
                <div class="flex items-center gap-3 border-b border-gray-100 bg-sky-50/60 px-4 py-3 dark:border-gray-700 dark:bg-sky-950/20">
                    <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-sky-500 text-white shadow-sm"><i class="fas fa-calendar-xmark"></i></span>
                    <div>
                        <p class="text-xs font-black text-sky-700 dark:text-sky-300">Siswa Tanpa Jadwal</p>
                        <p class="text-lg font-black text-slate-900 dark:text-white">{{ $kebersihan['siswa_tanpa_jadwal']->count() }}</p>
                    </div>
                </div>
                <div class="max-h-40 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($kebersihan['siswa_tanpa_jadwal'] as $s)
                        <div class="flex items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs dark:bg-gray-900/40">
                            ...
                            @if($s->kelas)<span class="ml-auto text-slate-400">{{ $s->kelas }}</span>@endif
                        </div>
                    @empty
            ...
            <div class="overflow-hidden rounded-2xl border border-t-4 border-t-fuchsia-400 border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:border-t-fuchsia-500 dark:bg-gray-800">
                <div class="flex items-center gap-3 border-b border-gray-100 bg-fuchsia-50/60 px-4 py-3 dark:border-gray-700 dark:bg-fuchsia-950/20">
                    <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-fuchsia-500 text-white shadow-sm"><i class="fas fa-phone-slash"></i></span>
                    <div>
                        <p class="text-xs font-black text-fuchsia-700 dark:text-fuchsia-300">Siswa Tanpa No. HP</p>
                        <p class="text-lg font-black text-slate-900 dark:text-white">{{ $kebersihan['siswa_tanpa_hp']->count() }}</p>
                    </div>
                </div>
                <div class="max-h-40 space-y-1.5 overflow-y-auto p-3">
            ...
            <div class="overflow-hidden rounded-2xl border border-t-4 border-t-stone-400 border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:border-t-stone-500 dark:bg-gray-800">
                <div class="flex items-center gap-3 border-b border-gray-100 bg-stone-50/60 px-4 py-3 dark:border-gray-700 dark:bg-stone-950/20">
                    <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-stone-500 text-white shadow-sm"><i class="fas fa-box-archive"></i></span>
                    <div>
                        <p class="text-xs font-black text-stone-700 dark:text-stone-300">Arsip Mengendap &gt; {{ $kebersihan['arsip_bulan'] }} Bulan</p>
                        <p class="text-lg font-black text-slate-900 dark:text-white">{{ $kebersihan['arsip_mengendap']->count() }}</p>
                    </div>
                </div>
                <div class="max-h-40 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($kebersihan['arsip_mengendap'] as $a)
                        <div class="flex items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs dark:bg-gray-900/40">
                            ...
                            <span class="ml-auto text-slate-400">sejak {{ \Carbon\Carbon::parse($a->created_at)->translatedFormat('d M Y') }}</span>
                        </div>
                    @empty
        ...
        @if ($kebersihan['guru_tidak_terpakai']->isNotEmpty() || $kebersihan['ruang_tidak_terpakai']->isNotEmpty() || $kebersihan['mapel_tidak_terpakai']->isNotEmpty())
            <div class="mt-4 grid grid-cols-1 gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:grid-cols-3">
                <div>
                    <p class="mb-2 flex items-center gap-1.5 text-xs font-black text-slate-500 dark:text-slate-400"><i class="fas fa-chalkboard-user text-slate-400"></i> Guru Tidak Terpakai ({{ $kebersihan['guru_tidak_terpakai']->count() }})</p>
                    <div class="flex flex-wrap gap-1">
                        @forelse ($kebersihan['guru_tidak_terpakai'] as $g)
                            <span class="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-600 dark:bg-slate-700 dark:text-slate-300">{{ $g->name }}</span>
                        @empty
                ...
                <div>
                    <p class="mb-2 flex items-center gap-1.5 text-xs font-black text-slate-500 dark:text-slate-400"><i class="fas fa-door-open text-slate-400"></i> Ruang Tidak Terpakai ({{ $kebersihan['ruang_tidak_terpakai']->count() }})</p>
                    <div class="flex flex-wrap gap-1">
                        @forelse ($kebersihan['ruang_tidak_terpakai'] as $r)
                            <span class="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-600 dark:bg-slate-700 dark:text-slate-300">{{ $r->name }}</span>
                        @empty
                            <span class="text-xs italic text-slate-400">-</span>
                        @endforelse
                    </div>
                ...
                <div>
                    <p class="mb-2 flex items-center gap-1.5 text-xs font-black text-slate-500 dark:text-slate-400"><i class="fas fa-book text-slate-400"></i> Mapel Tidak Terpakai ({{ $kebersihan['mapel_tidak_terpakai']->count() }})</p>
                    <div class="flex flex-wrap gap-1">
                        @forelse ($kebersihan['mapel_tidak_terpakai'] as $m)
                            <span class="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-600 dark:bg-slate-700 dark:text-slate-300">{{ $m->name }}</span>
                        @empty
                            <span class="text-xs italic text-slate-400">-</span>
                        @endforelse
                    </div>
