...
<div class="space-y-6">
        ...
        <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="space-y-2 max-h-72 overflow-y-auto pr-1">
                @forelse ($okupansi['data'] as $ruang)
                    <div>
                        <div class="flex justify-between gap-2 text-xs font-bold text-slate-600 dark:text-slate-300">
                            ...
                            <span class="shrink-0 whitespace-nowrap">{{ $ruang['terpakai'] }}/{{ $ruang['total_slot'] }}
                                ({{ $ruang['persentase'] }}%)
                            </span>
                        </div>
        ...
        <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="space-y-2 max-h-60 overflow-y-auto pr-1">
                @forelse ($beban['beban'] as $g)
                    <div class="flex justify-between gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300">
                        <span class="min-w-0 flex-1 truncate">{{ $g['nama'] }}</span>
                        <span class="shrink-0 whitespace-nowrap">{{ $g['jumlah_sesi'] }} sesi &middot; {{ intdiv($g['total_menit'], 60) }}j
                ...
                <div class="max-h-52 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($finansial['belum_ditagih'] as $item)
                        <div
                            ...
                            <span
                                class="min-w-0 flex-1 truncate font-bold text-slate-700 dark:text-slate-200">{{ $item['siswa_name'] }}</span>
                            <span class="ml-auto shrink-0 text-slate-400">{{ $item['paket'] }}</span>
                        </div>
                    @empty
                ...
                <div class="max-h-52 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($finansial['piutang_lama'] as $item)
                        <div
                            ...
                            <span
                                class="min-w-0 flex-1 truncate font-bold text-slate-700 dark:text-slate-200">{{ $item['siswa_names'] ?: $item['no_hp'] }}</span>
                            <span class="ml-auto shrink-0 font-black text-amber-600 dark:text-amber-400">Rp {{ number_format($item['total_sisa'], 0, ',', '.') }}</span>
                        </div>
                    @empty
                ...
                <div class="max-h-52 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($finansial['diskon_menggantung'] as $d)
                        <div
                            ...
                            <span class="min-w-0 flex-1 truncate font-bold text-slate-700 dark:text-slate-200">{{ $d->no_hp }}</span>
                            <span class="ml-auto shrink-0 text-slate-400">Rp {{ number_format($d->diskon, 0, ',', '.') }}</span>
                        </div>
                    @empty
                ...
                <div class="max-h-40 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($kebersihan['siswa_tanpa_jadwal'] as $s)
                        <div
                            ...
                            <span class="min-w-0 flex-1 truncate font-bold text-slate-700 dark:text-slate-200">{{ $s->name }}</span>
                            @if ($s->kelas)
                                <span class="ml-auto shrink-0 text-slate-400">{{ $s->kelas }}</span>
                            @endif
                        </div>
                    @empty
                ...
                <div class="max-h-40 space-y-1.5 overflow-y-auto p-3">
                    @forelse ($kebersihan['arsip_mengendap'] as $a)
                        <div
                            ...
                            <span class="min-w-0 flex-1 truncate font-bold text-slate-700 dark:text-slate-200">{{ $a->name }}</span>
    ...
    {{-- Jadwal hari ini: grid ala Jadwal Pembelajaran (Sesi x Hari Ini), klik kartu untuk detail siswa --}}
    <div x-data="{
                ...
                <table class="min-w-full w-full border-collapse table-fixed">
                    <thead class="bg-gray-100 dark:bg-gray-700/80">
                        <tr>
                            ...
                            <th
                                class="border border-gray-300 dark:border-gray-600 p-3 text-center uppercase text-xs tracking-wider font-semibold text-gray-600 dark:text-white min-w-[240px]">
                                <div class="text-base">{{ now()->translatedFormat('l') }}</div>
                                <span class="block mt-1 text-[10px] font-normal">{{ now()->translatedFormat('d F Y') }}</span>
                            </th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-gray-800">
                        <template x-for="grup in grouped" :key="grup.sesi_label">
                            <tr class="even:bg-gray-50/50 dark:even:bg-gray-800/60">
                                ...
                                <td class="border border-gray-200 dark:border-gray-600 p-2 align-top min-w-[240px]">
                                    <div class="space-y-2">
                                        <template x-for="(card, idx) in grup.items" :key="idx">
                                            <button type="button" @click="selectedCard = card"
                                                class="group flex w-full flex-col rounded-lg border-l-4 border-gray-100 bg-gray-50/60 p-2.5 text-left text-sm shadow-sm transition hover:shadow-lg hover:border-l-emerald-400 dark:border-gray-600 dark:bg-gray-700/70">
                                                <div class="flex items-center justify-between">
                                                    <strong
                                                        class="truncate font-bold text-gray-900 group-hover:text-emerald-600 dark:text-white dark:group-hover:text-emerald-400"
                                                        x-text="card.mapel_name"></strong>
                                                    <span
                                                        class="ml-2 inline-flex shrink-0 items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-black text-slate-600 dark:bg-slate-600 dark:text-slate-200">
                                                        <i class="fas fa-user-group"></i> <span
                                                            x-text="card.siswa_list.length"></span>
                                                    </span>
                                                </div>
                                                <span class="mt-1 block text-gray-600 dark:text-gray-200"
                                                    x-text="card.guru_name"></span>
                                                <span class="block text-xs text-gray-500 dark:text-gray-300"
                                                    x-text="'Ruang: ' + card.ruang_name"></span>
                                            </button>
                                        </template>
                                    </div>
                                </td>
                ...
                <div @click.stop x-transition
                    ...
                    <div
                        ...
                        <div class="min-w-0">
                            ...
                            <h3 class="mt-1 text-xl font-black truncate" x-text="selectedCard.mapel_name"></h3>
                        ...
                        <button type="button" @click="selectedCard = null"
                            class="shrink-0 rounded-full bg-white/15 px-3 py-2 text-sm font-bold hover:bg-white/25">Tutup</button>
                    </div>
