...
@section('content')
    <div class="mx-auto w-full max-w-7xl px-3 py-5 sm:px-6 lg:px-8 lg:py-8" x-data="calendarApp(@js(route('jadwal.kalender.export')))" x-init="init()">
        <div class="mb-6 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-500 p-5 text-white shadow-lg sm:p-7">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-2xl text-sm text-emerald-50">Cari jadwal berdasarkan hari, sesi, pelajaran, guru,
                        ruang, atau siswa.</p>
                </div>
                <a :href="exportUrl"
                    class="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-emerald-700 shadow hover:bg-emerald-50">
                ...
                @foreach ($haris as $hari)
                    <section
                        ...
                        <header class="flex items-center justify-between bg-gray-50 px-4 py-3 dark:bg-gray-900/60"
                            ...
                            <span
                                class="text-xs text-gray-500 dark:text-gray-400">{{ now()->startOfWeek()->addDays($loop->index)->translatedFormat('d M') }}</span>
                        </header>
                                    ...
                                    @foreach ($jadwals[$hari->id][$sesi->id] as $groupedClass)
                                        ...
                                        <article
                                            ...
                                            <div class="grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-gray-300">
                                                <span><i
                                                        class="fas fa-chalkboard-teacher mr-1 text-blue-500"></i>{{ $groupedClass['guru']->name }}</span>
                                                <span><i
                                                        class="fas fa-building mr-1 text-emerald-500"></i>{{ $groupedClass['ruang']->name }}</span>
                                            </div>
                                            <div class="mt-3 flex flex-wrap gap-1.5">
                                                @foreach ($groupedClass['siswa_list'] as $siswa)
                                                    <span
                                                        class="rounded-full bg-gray-100 px-2.5 py-1 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-200">{{ $siswa->panggilan ?: $siswa->name }}{{ $siswa->kelas ? ' · ' . $siswa->kelas : '' }}</span>
                                                @endforeach
                                            </div>
                                        </article>
                                    @endforeach
        ...
        <template x-if="isDesktop">
            <div
                class="overflow-x-auto rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <table class="min-w-[980px] w-full table-fixed border-collapse">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-900/60">
                            ...
                            @foreach ($haris as $hari)
                                <th class="border-b border-gray-200 p-3 text-sm dark:border-gray-700"
                        ...
                        @foreach ($sesis->sortBy('start_time') as $sesi)
                            <tr>
                                <th class="border-r border-t border-gray-200 p-3 align-top text-xs dark:border-gray-700">
                                    ...
                                    <span
                                        class="font-normal text-gray-500">{{ \Carbon\Carbon::parse($sesi->start_time)->format('H:i') }}–{{ \Carbon\Carbon::parse($sesi->end_time)->format('H:i') }}</span>
                                </th>
                                @foreach ($haris as $hari)
                                    <td class="h-36 border-t border-gray-200 p-2 align-top dark:border-gray-700">
                                        @foreach ($jadwals[$hari->id][$sesi->id] ?? [] as $groupedClass)
                                            ...
                                            <article
                                                ...
                                                <strong
                                                    class="block text-sm text-gray-900 dark:text-white">{{ $groupedClass['mapel']->name }}</strong>
                                                <span class="mt-1 block text-gray-600 dark:text-gray-300"><i
                                                        class="fas fa-chalkboard-teacher mr-1 text-blue-500"></i>{{ $groupedClass['guru']->name }}</span>
                                                <span class="block text-gray-500 dark:text-gray-400"><i
                                                        class="fas fa-building mr-1 text-emerald-500"></i>{{ $groupedClass['ruang']->name }}</span>
                                                <p class="mt-2 line-clamp-3 text-gray-500 dark:text-gray-300">
