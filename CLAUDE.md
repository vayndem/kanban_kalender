...
- **Config/route caching is now baked in at build time, not left for a cold start to generate.** `composer.json`'s `scripts.vercel` (a build hook `vercel-php` runs automatically) calls `php artisan config:cache` and `php artisan route:cache` during the build, writing to Laravel's normal `bootstrap/cache/*.php` — which is part of the deployed function bundle, not `/tmp`. `vercel.json` no longer redirects `APP_CONFIG_CACHE`/`APP_SERVICES_CACHE`/`APP_PACKAGES_CACHE`/`APP_ROUTES_CACHE` to `/tmp/*`; those redirects meant nothing ever actually got cached (nothing ever wrote to that ephemeral `/tmp` path, so every single request — cold *and* warm — re-parsed every config file and re-registered every route from scratch). `VIEW_COMPILED_PATH` is deliberately left pointed at `/tmp/views` — Blade compilation is per-view and self-healing if missing, so it stays on the safer runtime-writable path rather than being pre-baked. **This needs verifying on a real deploy** (not tested against the live Vercel project) — specifically, config-caching only bakes in correct values if `DB_*`/`APP_KEY`/etc. secrets are available to the Vercel *build* step, not only at runtime; if a deploy comes up with wrong DB config, this is the first thing to check, and `vercel.json`'s `env` block plus the `vercel` composer script are where to look.
