...
The app is split into several admin domains plus one public area, all routed through `routes/web.php`:
...
7. **Absen** (`ModulAjarController::absen`, `role:admin|guru`) — the day x session teaching board. A guru marks themselves unavailable and the slot opens to everyone at once (first come, first served); whoever actually teaches and grades earns the attendance credit.
8. **Payroll** (`PayrollController`, `PayrollService`) — per-guru rates, payslips, and the "Siap Lakukan" period close. Admin only. See "Planned direction" item 6 for the invariants.
...
**Services:** `PaymentBatchService` (mass billing / settlement), `RingkasanService` (dashboard summary aggregation), `RaporService` (per-student progress aggregation) and `PayrollService` (rates, payslips, period close) hold business logic that would otherwise bloat the controllers — check these before adding payment or dashboard-summary logic directly in a controller. `RingkasanService::kebersihanData()` includes a `tanda_lama` entry (notes older than 14 days, `TANDA_LAMA_HARI`) surfaced on the Ringkasan tab — it's a pure age check against `created_at`, there's no "resolved" flag on `Tanda`, so a note keeps showing up until someone deletes it.

## Frontend conventions

### Design system: Tailwind v4 + daisyUI v5, CSS-first

**There is no `tailwind.config.js` and no `postcss.config.js` — deleting them was the migration, not an accident.** Tailwind v4 is configured entirely from `resources/css/app.css` and compiled by the `@tailwindcss/vite` plugin in `vite.config.js`. The whole theme lives in that one stylesheet: `@import 'tailwindcss'`, `@source` for the Blade/JS scan paths, `@plugin 'daisyui' { themes: false }`, then two `@plugin 'daisyui/theme'` blocks (`eling` with `default: true`, `eling-dark` with `prefersdark: true`).

**Never write a hardcoded Tailwind palette color for chrome** (`bg-white`, `text-gray-500`, `border-gray-200`, …) — use the semantic tokens: `bg-base-100` (card/surface), `bg-base-200` (sunken/hover), `bg-base-300` (borders), `text-base-content` with an opacity suffix for de-emphasis, and `primary` / `accent` / `success` / `warning` / `error` / `info` / `neutral` for meaning.

**`text-base-content/60` is the opacity floor for readable text.** `/40` and `/50` measure 2.55:1 and 3.41:1 against `base-100` in the light theme — both fail WCAG AA. Use `/60` (4.69:1) or darker for anything a person actually reads; `/40` is fine for a decorative icon.

**Do not add `dark:` variants for token-based colors.** daisyUI emits the dark theme at `:root:not([data-theme])` inside `@media (prefers-color-scheme: dark)`, so tokens already flip. There is no `darkMode: 'class'` and nothing sets a `.dark` class. In JS, detect it with `isDarkMode()` from `resources/js/core/theme.js`, never `classList.contains('dark')`.

**The brand is emerald green — the owner has confirmed it twice.** `primary` is emerald and stays emerald. An earlier pass made the *whole theme* blue/purple and was rejected outright ("tema nya kenapa ungu? emerald hijau saja"), so **chrome** — nav, tabs, focus rings, brand gradients, the logo — must never go blue or purple again. That is a rule about chrome, not about the whole palette: `accent` is deliberately violet (see the next paragraph) because it is a *button role*, not chrome, and it has to be distinguishable from emerald.

**Each button role owns a distinct hue — do not collapse them back into one family.** The owner rejected an all-green toolbar outright ("tombol jangan aksen hijau semua, tombol krusial harus warna nya masing2"). `primary` emerald = default action, `accent` violet = supporting/management dialog, `info` sky = record/backup, `success` green = safe confirm & WhatsApp, `warning` amber = mass action needing care, `btn-export` rose = generate a document, `btn-sacred` deep rose = destructive/sensitive. `accent` must stay out of the green family; it was teal once and made every button look identical.

**Custom button and badge variants must be unlayered and set daisyUI's custom properties.** Two traps, both hit during the v5 migration: (1) daisyUI paints from `--btn-color` / `--btn-fg` (and `--badge-color` / `--badge-fg`), so setting `background-color` silently loses to `.btn`'s own declaration; (2) the compiled layer order is `properties, theme, base, components, utilities, daisyui` — **`daisyui` is last, and a cascade layer beats specificity outright**, so a rule inside `@layer components` can never win no matter how many classes you stack. That is why the `.btn.btn-*` / `.badge.badge-*` overrides sit at the very end of `app.css` outside any `@layer`. Moving them back inside a layer silently reverts every custom button colour.

**Solid green surfaces always carry white text, and that is deliberately decoupled from the tokens.** No single green can be both a good white-text button (>=4.5:1 against white) and a readable `text-primary` on the dark surface (>=4.5:1 against `base-100`) — the two requirements move in opposite directions. So the tokens stay light in the dark theme (good for `text-primary`), while `.btn.btn-primary` / `.btn.btn-success` / `.btn.btn-accent`, the matching `.badge.*`, `.brand-chip`, `.app-tab-active` and `.modal-header-brand` pin explicit dark greens (#047857 / #15803d / #0f766e) with white text in `app.css`, identical in both themes and measured at 5.0-5.5:1. Don't "simplify" these back onto `bg-primary`/`text-primary-content` — that reintroduces the dark-on-green buttons the owner rejected.

**v4 `@apply` only takes utilities.** It cannot apply a class you defined in `@layer components`, and it cannot apply a daisyUI component like `btn`. The four composable bases (`app-card`, `app-tab`, `icon-action`, `brand-chip`) are therefore declared with `@utility` at the top level so other rules can `@apply` them; `btn-export` and `btn-sacred` are modifiers that expect `btn` beside them in the markup (`class="btn btn-export"`).

**v4/v5 renames already applied — don't reintroduce the old names:** `rounded-btn` -> `rounded-field`, `rounded-badge` -> `rounded-selector`, `outline-none` -> `outline-hidden`, `flex-shrink-0` -> `shrink-0`, `flex-grow` -> `grow`, and the shadow/blur scale shifted one step (`shadow` -> `shadow-sm`, `shadow-sm` -> `shadow-xs`). daisyUI v5 dropped `input-bordered`/`select-bordered` (borders are the default now). v4's default border color is `currentColor`, so `app.css` restores a themed default with `*, ::before, ::after { border-color: var(--color-base-300) }` — don't "fix" bare `border` back to `border-gray-200`.

Where a name would collide with a daisyUI component (`card`, `badge`, `stat`, `tab`, `table`, `skeleton`, …) **daisyUI wins and ours is prefixed `app-`**: `.app-canvas`, `.app-card` / `.app-card-hover` / `.app-card-pad`, `.app-stat` / `.app-stat-value` / `.app-stat-label`, `.app-section-head` / `.app-section-title` / `.app-section-sub`, `.app-empty` + `-icon` / `-title` / `-text`, `.app-label`, `.app-input`, `.app-chip`, `.app-tab` / `.app-tab-active`, `.app-table-wrap`, `.brand-chip`. Use daisyUI's own `badge`, `table`, `join`, `alert`, `checkbox`, `select`, `input`, `modal`, `loading` directly rather than reinventing them.

Shared Blade components worth reusing before hand-rolling markup: `x-list-panel` + `x-list-row` (the tinted "header + count + scrolling list" cards on Ringkasan), `x-portal-nav` (the sticky top bar shared by Portal Guru / Absen / Modul Ajar), and the Breeze form components (`x-text-input`, `x-input-label`, `x-input-error`, `x-primary-button`, …), all of which are token-based.
...
**Responsive is verified by measurement, not by eye.** Every screen is checked at 375px and 768px for horizontal overflow (`documentElement.scrollWidth` vs `clientWidth`, ignoring anything inside its own `overflow-x` container) and for tap targets under 32px. Two real bugs came out of that audit: a `btn-xs sm:btn-sm` period toggle that gave *phones the smallest* size (backwards — size up for touch, not down), and a raw 16px checkbox that never got migrated to daisyUI. If you add a control, give it the larger size on mobile and only shrink it at `sm:` and up.

**A `transform` on an ancestor makes `position: fixed` resolve against that ancestor, not the viewport.** Wrapping the admin slot in an animated `<div>` once pushed every modal off-screen on long pages. Keep entry animations on leaf cards, never on a wrapper that contains a modal.
...
Recorded Sept 2026, kept as a record of how each item actually landed. Items 1, 2 and 6 are done (see "Roles and access" and item 6 below). Item 3 is also done, but not the way this section originally assumed — see below. The rest is intent — confirm details with the owner before building, since several items reshape the domain model.
...
- Teacher attendance: done, as `absensi_gurus`, but **not daily** as originally planned — the owner corrected this Sept 2026: it's credited per graded class session, so a teacher who completes 3 classes in a day earns 3, not 1. Payroll multiplies by `gaji_per_kehadiran` per credited session accordingly — it never assumes one credit per calendar day.

**6. Payroll — built (Sept 2026).** `PayrollService` + `PayrollController` + the `Payroll` tab. Rates live on `gurus` (`gaji_bawaan`, `gaji_per_kehadiran`), payslips in `penggajians`.

- **Runs are ad-hoc, not monthly.** The owner runs payroll whenever they like (often twice a month) and the base salary is paid **in full on every run** — it is not prorated. A guru with zero attendance still gets a payslip containing just the base salary; that is intended, not a bug.
- **"Siap Lakukan" closes the period, it does not delete.** It stamps every `absensi_gurus` row for that guru that has `penggajian_id IS NULL` with the new payslip's id plus `ditutup_pada`. The running counter is `WHERE penggajian_id IS NULL`, so it falls to zero on its own while the history stays intact. **Never implement the reset as a DELETE** — the payslip has to remain auditable, and later edits to old attendance must not silently change money that was already paid.
- **Rates are snapshotted onto the payslip.** Raising `gaji_per_kehadiran` later must not change last month's payslip, so `penggajians` stores its own copy of both rates plus the computed total.
- **Correcting a payslip is void-then-reissue, never edit.** `batalkan()` marks it cancelled (with actor, timestamp and a required reason) and releases its attendance rows back to `penggajian_id = null` so they count again on the next run.
- **Double-submit guard is time-based** (`PayrollService::JEDA_ANTI_GANDA`, 180s), not row-count-based — because a zero-attendance run is legitimate, so "no rows claimed" cannot be used to detect a duplicate.
- The claim itself is a single atomic `UPDATE ... WHERE penggajian_id IS NULL` whose affected-row count *is* the authoritative attendance figure, so two concurrent runs cannot double-pay.

Payroll inherits the payment module's rules because it is money: a recorded actor and timestamp on every payslip, no hard deletes (void/reverse instead), and idempotency against double submission. The "attendance is the input to pay" hazard is handled by the snapshot — once a payslip is issued, later edits to that attendance cannot change it, because the payslip carries its own copy of the rates, the count and the total.
